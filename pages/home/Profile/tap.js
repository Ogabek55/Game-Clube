...
document.addEventListener('DOMContentLoaded', () => {
    ...
    animateValue(counter1, 0, 2500, 2000, (value) => (value / 1000).toFixed(1) + 'k'); // 2.5k
    animateValue(counter2, 0, 1300, 2000, (value) => (value / 1000).toFixed(1) + 'k'); // 1.3k
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Funktsiya raqamlarni yangilash uchun
    function updateButtonWidth() {
        const widthPercentage = 80; // Yakuniy kenglik foizi
        let startWidth = 5; // Boshlang'ich kenglik foizi
        const duration = 2000; // Animatsiya davomiyligi (millisekund)
        const startTime = performance.now();

        function step(timestamp) {
            const progress = Math.min((timestamp - startTime) / duration, 1);
            const currentWidth = Math.floor(progress * (widthPercentage - startWidth) + startWidth);
    ...
    setTimeout(updateButtonWidth, 100); // Animatsiya boshlanishi uchun 100ms kechikish
});
